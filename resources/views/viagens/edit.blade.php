        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="container">
                    ...
                    <form action="{{ route('viagens.update', $viagem) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group mt-4">
                            ...
                            <select name="motoristas[]" id="motoristas" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" multiple>
                                @foreach($motoristas as $motorista)
                                    <option value="{{ $motorista->id }}" {{ $viagem->motoristas->contains($motorista->id) ? 'selected' : '' }}>
                                        {{ $motorista->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mt-4">
                            ...
                            <input type="number" name="km_inicial" id="km_inicial" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" value="{{ old('km_inicial', $viagem->km_inicial) }}">
                        </div>

                        <div class="form-group mt-4">
                            <label for="km_final">KM Final</label>
                            <input type="number" name="km_final" id="km_final" class="form-control rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 block mt-1 w-full" value="{{ old('km_final', $viagem->km_final) }}">
                        </div>

                        <button type="submit" class="btn btn-padrao mt-3">Atualizar Viagem</button>
                    </form>
                </div>
            </div>
        </div>
